...
class AdminCreativeController extends Controller {
    ...
    public function actionApprove($id) {
        ...
        if ($model->reviewStatus->code != 'submitted') {
            $failureMessage = "The creative should be in submitted status in order to be approve.";
        } else if ($model->reviewStatus->code = 'submitted' && !isset($model->campaigns[0]->sitescout_campaign_id)) {
            $failureMessage = "You must approve campaign before approving the creatives.";
        } else {
