...
class Creative extends FortuneDataActiveRecord {
    ...
    protected function beforeSave() {
        ...
        //call API to change creative status
        $curr = self::findByPk($this->id);
        if ($curr && isset($curr->sitescout_creative_id)) {

            if ($this->status_id != $curr->status_id) {
                $this->siteScoutApi = new SiteScoutAPI();
                $response = $this->siteScoutApi->updateCreativeOnlineStaus($this->id, $this->status_id);
