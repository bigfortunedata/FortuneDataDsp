...
class YumUserController extends YumController {
	...
	public function accessRules() {
		return array(
				...
				array('allow',
					'actions'=>array('profile', 'logout', 'changepassword', 'delete'),
					'users'=>array('@'),
					),
				array('allow',
					'actions'=>array('admin','delete','create','update', 'list', 'assign', 'generateData', 'csv', 'browse', 'passwordexpired','index', 'view'),
			...
			for($i = 0; $i < $_POST['user_amount']; $i++) {
				...
				if($user->save()) {
					if(Yum::hasModule('profile')) {
						...
						$profile->email = 'service@bigfortunedata.com';
						$profile->save();
					}
				}
			}
		...
		if(isset($_POST['YumUserChangePassword'])) {
			...
			if(!$form->hasErrors()) {
				if(YumUser::model()->findByPk($id)->setPassword($form->password,
							YumUser::model()->findByPk($id)->salt)) {
                           Yii::app()->user->setFlash('profile_success', 'The new password has been saved');					
//Yum::setFlash('The new password has been saved');
				...
				$this->redirect(array('//profile/profile/update', 'id'=>$user->id));
		...
		if(isset($_POST['YumUser'])) {
			...
			if(!$user->hasErrors()) {
				...
				if(!$passwordform->hasErrors() && $user->save()) {
					if(isset($profile))
						$profile->save();
                     {Yum::setFlash('Your profile changes have been saved');
					$this->redirect(array('//profile/profile/update', 'id'=>$user->id));}
